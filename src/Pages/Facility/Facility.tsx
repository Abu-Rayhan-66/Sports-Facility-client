...
const Facility = () => {
  ...
  return (
    <div className="lg:flex flex-1  mt-24">
      ...
      <div className="mb-6  mr-3 md:bg-white lg:bg-transparent">
        <input
          type="text"
  ...
  <button
    onClick={() => handlePageChange(currentPage + 1)}
    disabled={data && data.data.length < 10} // Disable if less than limit
      ...
      {isLoading ? (
        ...
        <div className="flex-[5] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mx-auto ">
          {data.data.map((item: Inputs) => (
            <div key={item._id} className="card card-compact bg-slate-300  rounded-md border-[#03AED2] border-[1px] shadow-sm hover:shadow-[#03AED2]">
              <figure>
                <img className="h-40 w-full" src={item.image} alt="Facility image" />
              </figure>
              <div className="card-body">
                <h2 className="text-lg font-medium text-white">{item.name}</h2>
                <p className="text-base text-white">Price per hour: {item.pricePerHour}</p>
